...
from flask import Flask, Response, session, request, url_for, render_template, redirect, send_file
...
redirefct_uri = 'http://localhost:5000/callback'
...
def download(artist, track):
    ...
    song_path = os.join(music_directory, song)

    # Send response to the client side
    response = send_file(song_path, as_attachment=True)
    response.headers['Content-Disposition'] = f'attachment; filename="{song}"'
